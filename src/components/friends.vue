  ...
  <div class="friends">
    ...
    <div class="friends-all">
      <div v-for="user in users[0]" class="user">
        <p>{{user.nick}}</p>
        <div class="opt">
          <!-- <span class='opt-l'>Принять запрос</span>
          <span class='opt-l'>Отклонить запрос</span> -->
          <span @click='startchat(user._id)'>start chat</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    // chat/user/outreq
    async startchat(id) {
      let response = await fetch('/chat/start', {
        method: 'POST',
        headers: {
    ...
    async friends() {
      ...
      let response = await fetch('/chat/user/friends', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
